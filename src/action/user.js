...
export const registration = (name, surname, phone, password) => {
    return async dispatch => {
        try {
            ...
            const registrationResponse = await axios.post('http://86.107.44.226:3001/api/auth/registration', {
                name,
                surname,
                phone,
                password
            ...
            const loginResponse = await axios.post('http://86.107.44.226:3001/api/auth/login', {
                phone,
                password
            });
...
export const login =  (phone, password) => {
    return async dispatch => {
        try {
            const response = await axios.post(`http://86.107.44.226:3001/api/auth/login`, {
                phone,
                password
            })
...
export const auth =  () => {
    return async dispatch => {
        try {
            const response = await axios.get(`http://86.107.44.226:3001/api/auth/auth`, 
